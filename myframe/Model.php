...
class Model
{
    ...
    {
        $sql = 'SELECT c.id, c.user_id,c.content,c.create_at,u.username
        FROM cms_comment AS c
        INNER JOIN cms_user AS u
        ON c.user_id = u.id
    ...
    public  function  getAllComments(){
        $sql='SELECT co.id, u.username, a.title,co.content,co.create_at
            FROM
                cms_comment co,
                cms_user u,
                cms_article a
            WHERE  co.article_id= a.id and
             co.user_id= u.id';
        return $this->db->fetchAll($sql);
    }
